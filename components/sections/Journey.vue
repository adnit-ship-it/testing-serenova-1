  ...
  <UiSectionWrapper class="flex-col py-20">
      ...
      <div ref="sectionRef">
        ...
        <div class="flex flex-row lg:flex-col gap-8 px-8 lg:px-0 max-w-[1168px] w-full">
          <div class="flex flex-col items-center lg:hidden py-10 relative">
            <div class="progress-line absolute top-12 w-[1px] max-h-[224px]"
              :style="{ height: progressHeight + 'px', backgroundColor: journey?.media?.progressLine?.color }"></div>
            <div class="h-2 w-2 md:h-3 md:w-3 rounded-full" :style="{ backgroundColor: journey?.media?.progressLine?.color }" />
            ...
            ]" :style="progress >= dotThresholds[1] ? { backgroundColor: journey?.media?.progressLine?.color } : {}" />
            <div class="h-[108px] md:h-[152px] w-[1px] md:w-[2px] bg-[#DBD9D9]"></div>
            <div :class="[
              'h-2 w-2 md:h-3 md:w-3 rounded-full transition-all duration-300',
              progress >= dotThresholds[2]
                ? 'border-0'
                : 'border-[#DBD9D9] border md:border-2',
            ]" :style="progress >= dotThresholds[2] ? { backgroundColor: journey?.media?.progressLine?.color } : {}" />
          </div>
    ...
    <div class="hidden lg:flex w-full h-[1px] bg-[#d9d9d9] mt-8  justify-center">
      <UiSectionContainer class="flex flex-row justify-around">
        <div v-for="(step, index) in journey?.steps" :key="index" class="h-2 w-2 rounded-full -mt-[3px]" :style="{ backgroundColor: journey?.media?.progressLine?.color }" />
      </UiSectionContainer>
    </div>
